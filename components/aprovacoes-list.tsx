...
export function AprovacoesList({ pedidos, tipoAcesso }: AprovacoesListProps) {
  ...
  const executarAcao = async (pedido: PedidoPagamento, acao: AcaoTipo, obs: string, dataPrevisaoPagamento?: string) => {
    ...
    try {
      ...
      alert(error instanceof Error ? error.message : "Erro ao processar ação. Tente novamente.")
    } finally {
      setProcessingId(null)
    }
  }

  const handleAprovarRapido = (pedido: PedidoPagamento) => {
    if (isSystemSuspended) {
      setSuspendedDialogOpen(true)
      return
    }
    if (!confirm("Confirma a aprovação deste pedido?")) return
    executarAcao(pedido, "aprovar", "")
  }
      ...
      <Dialog open={!!dialogAlvo} onOpenChange={(open) => !open && setDialogAlvo(null)}>
        <DialogContent className="sm:max-w-md">
          ...
          <DialogFooter>
            ...
            >
              {processingId === dialogAlvo?.pedido.id ? "Processando..." : "Confirmar"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
                      ...
                      <div className="flex items-center justify-end gap-1">
                        ...
                        >
                          <button
                            title="Aprovar"
                            onClick={() =>
                              tipoAcesso === "Financeiro" ? abrirDialog(pedido, "aprovar") : handleAprovarRapido(pedido)
                            }
